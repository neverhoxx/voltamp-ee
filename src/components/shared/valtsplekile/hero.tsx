...
import heroImg from '@/images/bgs/valtsplekk-bg.jpg';
...
interface ValtsplekileHeroProps {
    params: { locale: string };
}

export default function ValtsplekileHero({ params }: ValtsplekileHeroProps) {
    const { locale } = params;
            ...
            >
                <Container className="flex flex-col justify-center min-h-screen pt-[114px] text-center">
                    <h1 className="text-white font-bold text-[30px] sm:text-[32px] mb-2.5 text-center">
                        <span className="text-def">{t[`ValtsplekileSpan`]}</span> {t[`ValtsplekileHeroTitle`]}
                    </h1>

                    <p className="text-white text-[16px] font-medium text-center">
                        {t[`ValtsplekileHeroText`]}
                    </p>
