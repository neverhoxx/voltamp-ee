...
interface TrapetsprofiilplekileSection1Props {
    params: { locale: string };
}

export default function TrapetsprofiilplekileSection1({ params }: TrapetsprofiilplekileSection1Props) {
    const { locale } = params;
        ...
        >
            <Container className="flex flex-col items-center text-center">
                <h1 className="text-black font-bold text-[24px] sm:text-[30px] mb-5 text-left sm:text-center">
                    {t[`firstsectionTitle`]} <span className="text-def">{t[`firstsectionSpan`]}</span>
                </h1>
