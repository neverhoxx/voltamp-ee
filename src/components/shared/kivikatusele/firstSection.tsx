...
interface KivikatuseleSection1Props {
    params: { locale: string };
}

export default function KivikatuseleSection1({ params }: KivikatuseleSection1Props) {
    const { locale } = params;
        ...
        >
            <Container className="flex flex-col items-center text-center">
                <h1 className="text-black font-bold text-[24px] sm:text-[30px] mb-5 text-left sm:text-center">
                    {t[`firstsectionTitle`]} <span className="text-def">{t[`kivikatuselefirstsectionSpan`]}</span>
                </h1>

                <p className="text-black text-[16px] sm:text-[20px] font-semibold text-justify max-w-[900px]">
                    {t[`kivikatuselefirstsectionText`]}
                </p>
            </Container>
        </Element >
